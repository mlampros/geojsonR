...
testthat::test_that("the 'save_R_list_Features_2_FeatureCollection' function works as expected (Be aware that the sample input matrices MUST have 2 columns to resemble lat-lon values, otherwise an error will be raised)", {
  ...
  #............................................................................
  # Matrices that are used in the following polygon and multipolygon geometries
  #............................................................................

  DIGITS = 5
  set.seed(1)

  mt_20_2_dims = matrix(round(runif(20), digits = DIGITS), nrow = 10, ncol = 2)
  mt_8_2_dims = matrix(round(runif(8), digits = DIGITS), nrow = 4, ncol = 2)
  ...
  multipolygon_WITH_interior = list(type ="Feature",
                                    ...
                                    geometry = list(type = 'MultiPolygon',
                                                    coordinates = list(list(mt_20_2_dims,                 # one or more polygons with interior rings
                                                                            mt_8_2_dims),
                                                                       mt_20_2_dims,                      # one or more polygons without interior rings
                                                                       mt_20_2_dims)))
  ...
  TOLERANCE = as.numeric(paste(c('0.', paste(rep(0, DIGITS-1), collapse = ''), '1'), collapse = ''))

  cat('\nUsing a tolerance value of: ', TOLERANCE, '\n')

  plg_wo_int = all.equal(polygon_WITHOUT_interior[feat_nams], res_load$features[[which(prop_names == 'polygon-without')]][feat_nams], tol = TOLERANCE)      # add the tolerance parameter for differences that might arise due to save and load procedure
  plg_with_int = all.equal(polygon_WITH_interior[feat_nams], res_load$features[[which(prop_names == 'polygon-with')]][feat_nams], tol = TOLERANCE)
  mlt_plg_wo_int = all.equal(multipolygon_WITHOUT_interior[feat_nams], res_load$features[[which(prop_names == 'multipolygon-without')]][feat_nams], tol = TOLERANCE)
  mlt_plg_with_int = all.equal(multipolygon_WITH_interior[feat_nams], res_load$features[[which(prop_names == 'multipolygon-with')]][feat_nams], tol = TOLERANCE)

  verify_plgs = rep(FALSE, 4)

  lst_res = list(plg_wo_int, plg_with_int, mlt_plg_wo_int, mlt_plg_with_int)
  vec_nams = c('polygon-without', 'polygon-with', 'multipolygon-without', 'multipolygon-with')

  for (item in 1:length(lst_res)) {
    cat('The  ', vec_nams[item], '  geometry returned:', lst_res[[item]], '\n')

    if (is.logical(lst_res[[item]])) {
      verify_plgs[item] = lst_res[[item]]
    }
  }

  idx_error = which(!verify_plgs)
  if (length(idx_error) > 0) {
    cat('The  ', vec_nams[idx_error], '  geometry gave an Error! Take a look! See if it is related with the "tolerance" parameter!\n')
  }
  ...
  testthat::expect_true( all( verify_plgs ) )
})
